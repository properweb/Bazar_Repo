...
class BrandProductController extends Controller
{
    ...
    public function ProductSort(Request $request)
    {
        ...
        if ($existRedis > 0) {
            ...
            $fetchProduct = json_decode($cachedProducts, false);
            $response = ['res' => true, 'msg' => "", 'data' => $fetchProduct];
        } else {
            ...
            $fetchProduct = Redis::set("brandproduct:fetchproductsort:" . $request->page . ":" . $search . ":" . $request->status . ":" . $request->sort_key . ":" . $request->user_id, json_encode($data));
    ...
    public function fetchProducts(Request $request)
    {
        ...
        if ($existRedis > 0) {
            ...
            $productVariationsTbl = DB::raw("(SELECT product_id,id as variant_id,value1,value2,value3,sku as vsku,stock as vstock,image as vimage
		FROM product_variations WHERE status='1') as pv");// Raw query is needed as nested query using for this function with alias.
            $products_sql = DB::table('products as p')
                ->select('p.*', 'pv.*')
                ->leftjoin($productVariationsTbl, 'pv.product_id', '=', 'p.id')
