...
class AnalyticService
{
    ...
    public function totalSalesDates(object $request): array
    {
        ...
        $totalSell = [];
        $groupBy = 'day';
        $Sales = Order::selectRaw("DATE_FORMAT(created_at, '%m/%d') as date, SUM(total_amount) as totalSales")->where('brand_id', $userId)->whereBetween('created_at', [$request->from_date, $request->to_date])->groupBy(DB::raw("DATE_FORMAT(created_at, '%{$groupBy}')"))->orderBy('created_at', 'ASC')->get();
        if (!empty($Sales)) {
            foreach ($Sales as $sale) {
    ...
    public function traffic(object $request): array
    {
        ...
        $sales = Visit::selectRaw("DATE_FORMAT(created_at, '%m/%d') as date, SUM(orders) as totalSales,(count(DISTINCT(ip_address))) as counts ")->where('brand_id', $userId)->whereBetween('created_at', [$request->from_date, $nextDate])->groupBy(DB::raw("DATE_FORMAT(created_at, '%{$groupBy}')"))->orderBy('created_at', 'ASC')->get();
    ...
    public function fetchSellThrough(Request $request): array
    {
        ...
        $returnedReasons = ReturnItem::selectRaw('return_reasons.title, COUNT(return_items.reason_id) as used_count')
            ...
            ->where('orders.brand_id', $userId)
            ->groupBy('return_items.reason_id')->orderBy('used_count', 'DESC')->take(5)->get();

        $returnedComments = OrderReturn::selectRaw('returns.feedback as comment')
            ->join('orders', 'returns.order_id', '=', 'orders.id')
            ->where('orders.brand_id', $userId)
        ...
        $returnOrders = Order::join('returns', 'orders.id', '=', 'returns.order_id')
            ->where('orders.brand_id', $userId)
            ->whereRaw("DATE(returns.created_at) >= '".$request->from_date."'")
            ->whereRaw("DATE(returns.created_at) <= '".$request->to_date."'")
            ->get(['orders.id as order_id', 'orders.user_id as customer_id', 'returns.id as return_id', 'orders.created_at as order_date', 'orders.total_amount', 'orders.quantity as ordered_items', 'orders.order_number']);
        ...
        if (!empty($returnOrders)) {
            ...
            array_multisort($topReturnedItems, SORT_DESC, $sort_array);
        }
